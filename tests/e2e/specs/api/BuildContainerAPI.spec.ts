...
`;
	...
	suiteTeardown('Restore DevSpaces container build capabilities setting', function (): void {
		...
		const restorePatchCommand: string = `oc patch checluster/${cheClusterName} -n ${devSpacesNamespace} --type=merge -p '{"spec":{"devEnvironments":{"disableContainerBuildCapabilities":${originalBuildCapabilitiesSetting}}}}'`;
